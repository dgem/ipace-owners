(function () {
  ...
  function setupAdminResults(root) {
    ...
    function render(analysis) {
      root.innerHTML = '<article class="dashboard-panel"><div class="dashboard-panel__header"><div><p class="dashboard-panel__eyebrow">Admin-only analysis</p><h2 class="dashboard-panel__title">' + esc(analysis.survey.title) + '</h2><p class="form-hint">Masked respondent emails and free-text are visible only to administrators. The CSV contains no full emails, names, or Firebase IDs.</p></div><button class="btn btn--primary" type="button" data-survey-download>Download CSV</button></div><div class="survey-results"><div class="survey-results__header"><h3>Aggregate results</h3><span>' + analysis.total + ' response' + (analysis.total === 1 ? '' : 's') + '</span></div>' + analysis.survey.options.map(function (option, index) { var preferred = (analysis.preferredCounts && analysis.preferredCounts[option.id]) || 0; return '<div class="survey-result"><span class="survey-choice__number" aria-hidden="true">' + (index + 1) + '</span><div class="survey-markdown">' + markdown(option.label) + '</div><strong class="survey-result__count">' + ((analysis.counts && analysis.counts[option.id]) || 0) + (analysis.survey.multiple ? '<span class="survey-result__preferred">' + preferred + ' preferred</span>' : '') + '</strong></div>'; }).join('') + '</div><h3 class="survey-analysis__title">Individual submissions</h3><div class="table-wrapper"><table class="data-table"><thead><tr><th>Masked respondent</th><th>Submitted</th><th>Selected option IDs</th><th>Preferred option ID</th><th>Text responses</th></tr></thead><tbody>' + analysis.responses.map(function (response) { return '<tr><td>' + esc(response.respondent) + '</td><td>' + esc(response.updatedAt) + '</td><td>' + esc((response.optionIds || []).join(' · ')) + '</td><td>' + esc(response.preferredOptionId || '') + '</td><td>' + esc((response.textResponses || []).join(' · ')) + '</td></tr>'; }).join('') + '</tbody></table></div><p class="form-hint" role="status" aria-live="polite"></p></article>';
    ...
    function render(all) {
      ...
      active.forEach(function (result) {
        ...
        s.options.forEach(function (o, index) {
          ...
          choices.insertAdjacentHTML('beforeend', '<div class="survey-choice"><label><input type="' + (s.multiple ? 'checkbox' : 'radio') + '" name="survey" value="' + esc(o.id) + '" ' + (selected ? 'checked' : '') + '><span class="survey-choice__number" aria-hidden="true">' + (index + 1) + '</span><span class="survey-markdown survey-choice__copy">' + markdown(o.label) + '</span></label>' + (s.multiple ? '<div class="survey-choice__preferred" ' + (selected ? '' : 'hidden') + '><label><input type="checkbox" data-preferred-option value="' + esc(o.id) + '" ' + (preferred ? 'checked' : '') + '> Make this my preferred outcome</label></div>' : '') + (o.allowsText ? '<div class="survey-choice__text" ' + (selected ? '' : 'hidden') + '><label for="survey-text-' + esc(o.id) + '">Tell us more <span>(up to 250 characters)</span></label><textarea id="survey-text-' + esc(o.id) + '" data-option-text-id="' + esc(o.id) + '" maxlength="250" rows="3" placeholder="Please describe">' + esc(text) + '</textarea></div>' : '') + '</div>');
        });
        ...
        form.onchange = function (event) { Array.prototype.forEach.call(form.querySelectorAll('[data-option-text-id]'), function (field) { field.closest('.survey-choice__text').hidden = !form.querySelector('input[name="survey"][value="' + field.getAttribute('data-option-text-id') + '"]').checked; }); Array.prototype.forEach.call(form.querySelectorAll('[data-preferred-option]'), function (field) { var selectedOption = form.querySelector('input[name="survey"][value="' + field.value + '"]').checked; field.closest('.survey-choice__preferred').hidden = !selectedOption; if (!selectedOption) field.checked = false; }); if (event.target.matches('[data-preferred-option]') && event.target.checked) Array.prototype.forEach.call(form.querySelectorAll('[data-preferred-option]'), function (field) { if (field !== event.target) field.checked = false; }); };
        form.onsubmit = function (e) { e.preventDefault(); var selected = Array.prototype.map.call(form.querySelectorAll('input[name="survey"]:checked'), function (input) { return input.value; }), preferred = form.querySelector('[data-preferred-option]:checked'), textByOption = {}; selected.forEach(function (id) { var field = form.querySelector('[data-option-text-id="' + id + '"]'); if (field) textByOption[id] = field.value; }); request('/api/member/survey-response', 'POST', { surveyId: s.id, optionIds: selected, preferredOptionId: preferred ? preferred.value : '', textByOption: textByOption }).then(function () { window.location.assign('/member/survey-results/?id=' + encodeURIComponent(s.id)); }).catch(function (err) { article.querySelector('[role=status]').textContent = err.message; }); };
  ...
  function resultMarkup(result) {
    ...
    return '<article class="dashboard-panel survey-member-card"><p class="dashboard-panel__eyebrow">' + (submitted ? 'Response saved' : 'Survey closed') + '</p><h2 class="survey-member-card__title">' + esc(s.title) + '</h2><p class="survey-member-card__status">' + (submitted ? 'Thank you — your response is included below.' : 'Results are now available to all members.') + '</p><div class="survey-results"><div class="survey-results__header"><h3>Current results</h3><span>' + result.total + ' response' + (result.total === 1 ? '' : 's') + '</span></div>' + s.options.map(function (o, index) { var preferred = (result.preferredCounts && result.preferredCounts[o.id]) || 0; return '<div class="survey-result"><span class="survey-choice__number" aria-hidden="true">' + (index + 1) + '</span><div class="survey-markdown">' + markdown(o.label) + '</div><strong class="survey-result__count">' + ((result.counts && result.counts[o.id]) || 0) + (s.multiple ? '<span class="survey-result__preferred">' + preferred + ' preferred</span>' : '') + '</strong></div>'; }).join('') + '</div><div class="cluster survey-results__actions">' + (result.canRespond && submitted ? '<a class="btn btn--secondary" href="/member/survey-response/?id=' + encodeURIComponent(s.id) + '">Edit your response</a>' : '') + '<a class="btn btn--secondary" href="/member/surveys/?filter=closed">Closed surveys</a></div></article>';
  }
