...
Functions, Cloud Storage, Cloud Monitoring, Vertex AI, Secret Manager and GitHub Workload Identity Federation. It also
...
Open the **I-PACE Owners production operations** dashboard in Google Cloud Monitoring to investigate
an alert. Check the relevant uptime result, the serialized production deployment and smoke test,
then `Api` logs. Hosting normally continues serving the previous release until a replacement is
complete, so routine deploys do not take the site down. A failed smoke test does not automatically
roll back a completed release: use Firebase Hosting release history to roll back after confirming
the earlier release is healthy, then rerun the smoke test.
...
- `auth-diagnostics` records only bounded passwordless lifecycle events under an opaque,
  session-scoped support code. The code is carried through the passwordless email-link hand-off
  and attached to every authenticated API request. The shared server authorization guards log
  the traced route, required role, allow/deny decision, and HTTP status, letting support
  correlate a member report without collecting their email address, Firebase token, browser URL,
  or exception text.
