...
test('launch FAQ sets UK-led scope and keeps legal participation deliberate', function () {
  ...
  assert.match(faq, /For launch, we only ask for your name, email address/);
  assert.match(faq, /At launch we are only asking owners to register/);
...
test('about page describes aligned advocacy rather than an open forum', function () {
  ...
  assert.match(about, /For launch, we are asking owners only to register/);
  assert.doesNotMatch(about, /Submit Vehicle Data/);
