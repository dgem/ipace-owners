...
test('clean-room route inventory maps to source pages and Hosting redirects', function () {
  ...
  const routes = [
    ...
    ['/member/survey-history/', 'src/member/survey-history.njk'],
...
test('clean-room API and Firestore inventories match implemented handlers', function () {
  ...
  assert.match(goSource, /const publicStatsSchemaVersion = 6/);
  assert.match(contract, /`schemaVersion: 6`/);
  assert.match(read('src/assets/js/public-stats.js'), /\/api\/public-stats\?v=6/);
