...
| `POST /api/admin/survey-insights` | `AdminSurveyInsights` | Admin | Page through survey responses in stable document-ID order, redact common identifiers from optional comments, submit selected/preferred choice context to Vertex Gemini, and validate comment-level sentiment/themes and source-exact quote candidates. No identities or responses are logged or persisted. |
