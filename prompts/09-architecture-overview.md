...
| `GET` / `POST /api/email-unsubscribe` | `MarketingMessageUnsubscribe` | Public signed link | Withdraw contact consent from any matching Join record and the hashed communications-preference registry identified by an opaque per-delivery token; never expose the recipient email or require sign-in. |
...
| `GET /api/admin/survey-results` | `AdminSurveyResults` | Admin | Return count-only aggregate selected, preferred and optional-comment results plus a paginated, masked individual-response page for manual analysis; `format=csv` exports PII-minimised rows. Audit analysis views and CSV downloads only with the response total. |
| `POST /api/admin/survey-insights` | `AdminSurveyInsights` | Admin | Page through survey responses in stable document-ID order in batches of 24; classify de-identified comments in model groups of at most eight with three concurrent calls. Submit selected/preferred choice context to Vertex Gemini and validate comment-level sentiment/themes, optional controlled phrase/polarity tags, and source-exact quote candidates tagged with source option and sentiment. Invalid optional phrase tags are dropped; invalid model classifications trigger recursive smaller-group retries with fresh indices. Irreducible comments are explicitly unclassified and excluded from sentiment/themes/quotes, while transport errors still fail the page. The browser retries transient failures at the same offset and can resume from completed pages in memory. No identities or raw response documents are logged or stored by this route. |
| `POST /api/admin/survey-insights-summary` | `AdminSurveyInsightSummary` | Admin | Summarise checked comment classifications and produce three editable JLR discussion actions; browser-generated PowerPoint combines exact survey choice totals with consent-filtered aggregate demographics and reviewed quotes, then an admin-only archive saves the completed report and each deck version. |
| `GET/POST /api/admin/survey-insight-archive` | `AdminSurveyInsightArchive` | Admin | List saved analysis/deck metadata, restore one completed redacted AI report, or save a new immutable analysis snapshot. Check current aggregate counts before saving; store metadata in Firestore and report JSON in the private snapshot bucket. |
| `GET/POST /api/admin/survey-insight-deck` | `AdminSurveyInsightDeck` | Admin | Download a saved PPTX or save an immutable browser-generated version against an analysis. Validate three distinct quote indices per editorial category, or all available when fewer than three exist (including zero), against saved candidates, edited actions, size and ZIP structure; store metadata in Firestore and binary in the private snapshot bucket. |
...
- Firestore is canonical for structured owner, membership, vehicle, evidence, and review
  data.
- Cloud Storage holds generated public/member JSON snapshots and private completed survey
  insight JSON/PPTX versions; future evidence blobs will use authenticated storage.
...
Scan `serviceEvents`, then batch-get only their related `vehicles` and `members` documents,
using identity fields only for text redaction. Return a private/no-store CSV of non-deleted service records
