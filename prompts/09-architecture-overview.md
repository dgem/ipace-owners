...
| `POST /api/admin/marketing-message-send` | `AdminMarketingMessageSend` | Admin | Recalculate that consented audience and, only after the exact confirmation and unchanged count, send the next resumable batch of at most 100 individual Resend emails. A Firestore ledger makes delivery idempotent; a prepared template's stable source identity, not dynamic aggregate values in its rendered copy, identifies the campaign. Only name/email are transferred and each message has an opaque, recipient-specific unsubscribe link. |
