...
| `POST /api/admin/survey-insights` | `AdminSurveyInsights` | Admin | Page through survey responses in stable document-ID order in batches of 12 to stay below proxy timeouts, redact common identifiers from optional comments, submit selected/preferred choice context to Vertex Gemini, and validate comment-level sentiment/themes and source-exact quote candidates. The browser retries transient failures at the same offset and can resume from completed pages in memory. No identities or responses are logged or persisted. |
