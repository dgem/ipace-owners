...
| `GET/POST /api/admin/survey-insight-deck` | `AdminSurveyInsightDeck` | Admin | Download a saved PPTX or save an immutable browser-generated version against an analysis. Validate exactly three distinct good, bad and ugly quote indices against saved candidates, edited actions, size and ZIP structure; store metadata in Firestore and binary in the private snapshot bucket. |
